...
bool flashWrite(uint32_t addr, uint8_t *p_data, uint32_t length)
{
  ...
  for (int i=0; i<length; i+=32)
  {
    ...
    uint32_t copy_len = remain >= 32 ? 32 : remain;

    memcpy(flashWord, &p_data[i], copy_len);

    status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, addr + i, (uint32_t)flashWord);
    if (status != HAL_OK)
    {
      ret = false;
      break;
...
HAL_StatusTypeDef FLASH_ProgramWord(uint32_t Address, uint32_t Data)
{
  ...
  status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, Address, (uint64_t)Data);
